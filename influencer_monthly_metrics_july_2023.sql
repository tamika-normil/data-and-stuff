...
create temp table shops as 
   ( select distinct shop_id, shop_name, user_id as seller_user_id
   from `etsy-data-warehouse-prod.listing_mart.listing_vw` 
   where regexp_contains(title, r'John Legend Creator Collab'));
...
create or replace table `etsy-data-warehouse-dev.rollups.influencer_creator_drops_shops_jl` as 
(with shop_gms as   
  (SELECT lv.date, shop_id, 
  coalesce(v.top_channel,'') as top_channel,
  coalesce(v.second_channel,'') as second_channel,
...
SELECT date_trunc(date, week) as week, sum(shop_visits) as shop_visits, sum(shop_gms) as shop_gms, sum(attr_gms) as attr_gms
 FROM `etsy-data-warehouse-dev.rollups.influencer_creator_drops_shops_jl` 
