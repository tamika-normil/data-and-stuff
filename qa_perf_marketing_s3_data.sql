-- check whether this fixes meta campaign
select sum(visits_m) as visits, sum(attributed_gms) as attributed_gms, sum(attributed_gms_estimated) as attributed_gms_estimated, 'dev' as type
from `etsy-data-warehouse-dev.buyatt_mart.perf_marketing_s3_data` 
where utm_campaign_cleaned = 'us_meta_daba_eb_jewelry_and_bau_na'
union all
select sum(visits_m) as visits, sum(attributed_gms) as attributed_gms, sum(attributed_gms_estimated) as attributed_gms_estimated, 'prod' as type
from `etsy-data-warehouse-prod.buyatt_mart.perf_marketing_s3_data` 
where utm_campaign_cleaned = 'us_meta_daba_eb_jewelry_and_bau_na'
union all 
select sum(visits) as visits, sum(attributed_gms) as attributed_gms, sum(attributed_gms_adjusted) as attributed_gms_estimated, 'prod-channel-overview' as type
from `etsy-data-warehouse-prod.buyatt_rollups.channel_overview` a
where utm_campaign = 'us_meta_daba_eb_jewelry_and_bau_na'
...
WHERE purchase_date>= DATE_SUB(current_date(), INTERVAL 31 DAY)
and (a.second_channel in ('facebook_disp','instagram_disp','facebook_disp_intl','gpla', 'google_ppc', 'bing_ppc', 'css_plas', 'intl_css_plas',
	'bing_plas','intl_gpla','intl_ppc','intl_bing_ppc','intl_bing_plas','pinterest_disp','pinterest_disp_intl','us_video','intl_video', 'native_display', 'intl_native_display')
	or (a.utm_source='admarketplace' and a.utm_medium='cpc'))
# utm_campaign,
