...
with receipt_data as (
    ...
    where date(a.creation_tsz) >= current_date - 120
    ),
  payment_data as (
    select payment_id,
      receipt_group_id,
    ...
    where create_date >=  unix_seconds(timestamp(current_date - 120))
    ),
  cc_data as (
...
create or replace table etsy-data-warehouse-dev.tnormil.receipt_level_take_rate as 
  with receipts as (
    select rg.receipt_id,
      ...
      (coalesce(c.ep_processing_fee, 0) + (a.ep_revenue + a.fx_hedge_revenue)*0.15) as ep_processing_fees, 
